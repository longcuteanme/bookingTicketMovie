...
import { STATUS_CODE, USER_ACCESS_TOKEN, USER_INFO } from '../../utils/constants/settingSystem';
import { CAP_NHAT_THONG_TIN_NGUOI_DUNG_SAGA, DANG_NHAP_SAGA, DISPLAY_LOADING, DISPLAY_QUAN_TRI_LOADING, HIDE_LOADING, HIDE_QUAN_TRI_LOADING, LAY_DANH_SACH_NGUOI_DUNG_PHAN_TRANG_SAGA, LAY_DANH_SACH_NGUOI_DUNG_QUAN_TRI, LAY_THONG_TIN_TAI_KHOAN, LAY_THONG_TIN_TAI_KHOAN_SAGA, THEM_NGUOI_DUNG_SAGA, XOA_NGUOI_DUNG_SAGA } from '../constants/totalConstants';
...
function * dangNhap(action){
    ...
    try{
        ...
        if(status===STATUS_CODE.SUCCESS){
            yield localStorage.setItem(USER_ACCESS_TOKEN,JSON.stringify(data.content.accessToken))
...
function * capNhatThongTinNguoiDung(action){
    ...
    try{
        let {status} = yield call(QuanLyNguoiDungService.capNhatThongTinNguoiDung,model);
        if(status===STATUS_CODE.SUCCESS){
    ...
    yield action.payload.history?action.payload.history():action.payload.layDuLieu()
...
export function * sagaCapNhatThongTinNguoiDung(){
    yield takeLatest(CAP_NHAT_THONG_TIN_NGUOI_DUNG_SAGA,capNhatThongTinNguoiDung)  
}
...
function * layDanhSachNguoiDungPhanTrang(action){
    ...
    const model=action.payload.model
    try{
        let {data,status} = yield call(QuanLyNguoiDungService.layDanhSachNguoiDungPhanTrang,model);
        if(status===STATUS_CODE.SUCCESS){
            yield put({
...
export function * sagaLayDanhSachNguoiDungPhanTrang(){
    yield takeLatest(LAY_DANH_SACH_NGUOI_DUNG_PHAN_TRANG_SAGA,layDanhSachNguoiDungPhanTrang)  
}
...
function * themNguoiDung(action){
    ...
    try{
        ...
        if(respond.status===STATUS_CODE.SUCCESS){
            ...
            yield console.log('respond',respond)
        }
    }
    catch(error){
        const err={...error}
